...
void Plot::Init(OutputType outputType) {
	...
	// do not need to initialise if output type is PPM
}
 ...
 */
void Plot::Draw(OutputType outputType) { //, bool data2file, bool file2data) {
	switch (outputType) {
	case PL_GRAPH: DrawPLPlot(); break; //data2file, file2data); break;
	case PL_GRID: DrawPPM(); break;
	default:
...
void Plot::DrawPPM() {
	...
	for (int p = 0; p < data_v.size(); ++p) {
		FILE *stream;
		stringstream filenm; filenm.clear(); filenm.str("");
		filenm << path << ppm_file << p << ".ppm";
		stream = fopen(filenm.str().c_str(), "wb" );
		int len = sqrt(GetData(p).size());
		fprintf( stream, "P6\n%d %d\n255\n", len, len);
		...
		for(int j = 0; j < len; ++j) {
			for (int i = 0; i < len; ++i) {
				float value = GetData(p).item< float >(i+j*len) * nof_colors;
				if(value < 0) {
#ifdef ENFORCE_LIMITS
					cerr << "Enforce limits: negative value" << endl;
					assert(false);
#endif
					rgb[0] = 0;
					rgb[1] = 0;
...
#ifdef ENFORCE_LIMITS
					cerr << "Enforce limits: too large value" << endl;
					assert(false);
#endif
						rgb[0] = 0;
						rgb[1] = 0;
 ...
 */
void Plot::DrawPLPlot() { //bool data2file, bool file2data) {
	if (data_v.empty()) {
